...
<span class="token punctuation">}</span><span class="token punctuation">)</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>В момент инициализации <code v-pre>state</code> сначала будет обернут в <code v-pre>proxy</code> объект (зачем это сделано, вы узнаете позже) и
только затем
